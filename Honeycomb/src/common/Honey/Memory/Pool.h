...
private:
    ...
    struct BlockHeader
    {
        ...
        static const int sigUsed = 0x05EDB10C;      ///< Used Block
            ...
            void validate(int sig)
            {
                ...
                assert(debug.sig == sig, "Error: invalid block signature. Block not in expected state (ex. block freed twice).");
            }
        #else
            void validate(int) {}
        #endif
    ...
    public:
        ...
        int blockStride() const                             { return (int)alignCeil((void*)(_blockSize + sizeof(BlockHeader)), _pool._blockAlign); }
...
/// MemPool allocator.  Subclass must define std::allocator constructors and pool().
