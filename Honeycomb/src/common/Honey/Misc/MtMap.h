...
template<class KeyT, class ValT, class ListT> class MtMapElem;
...
public:
    ...
    template<class Iter> bool operator==(const Iter& rhs) const     { return _head == rhs._head && std::is_same<Pair::Key, typename Iter::Pair::Key>::value; }
...
template<class Subclass, class KeyT, class ValT, class ListT>
class MtMapCommon
{
...
public:
    typedef KeyT Key;   ///< Key type
    typedef ValT Val;   ///< Value type
    typedef ListT List; ///< Rest of map
...
private:
    ...
    struct priv
    {
        ...
        struct findElem                                             : List::template findElem<Key, Subclass> {};

        /// Specialize for our key
        template<class Prev>
        struct findElem<Key, Prev>
        {
        ...
        /// Recurse to tail
        template<int Count> struct sizeR                            : List::template sizeR<Count+1> {};
    };
...
/**
  ...
  */  
template<class KeyT, class ValT, class ListT>
class MtMapElem : public MtMapCommon<MtMapElem<KeyT,ValT,ListT>, KeyT, ValT, ListT>, public ListT
{
    friend class MtMapElem;
    template<class, class> friend class MtMapIter;
    typedef MtMapCommon<MtMapElem, KeyT, ValT, ListT> Super;
...
private:
    ...
    struct priv
    {
        ...
        static const bool movable = !mt::isLref<Map>::value && !mt::isRef<Val>::value && !mt::isRef<typename MapT::getResult<Key>::Type>::value;
