...
public:
    ...
    /// To string
    friend StringStream& operator<<(StringStream& os, const variant& rhs)   { rhs.visit(toString(), os); return os; }
...
private:
    ...
    struct toString { template<class T> void operator()(T&& val, StringStream& os) { os << val; } };
