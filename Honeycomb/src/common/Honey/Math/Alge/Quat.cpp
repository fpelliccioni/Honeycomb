...
template<class Real>
Quat_<Real>::Quat_(const Ray& ray, const Vec3& axis, const Vec3& target)
{
    Plane axisPlane(axis, target);
    Vec3 ray_p_proj = Geom::project(axisPlane, ray.origin);
    Vec3 ray_p2_proj = Geom::project(axisPlane, ray.origin+ray.dir);
    fromAlign((ray_p2_proj-ray_p_proj).normalize(), (target-ray_p_proj).normalize());
}
