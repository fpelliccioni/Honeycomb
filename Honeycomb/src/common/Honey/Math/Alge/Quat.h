...
template<class Real> class Plane_;
template<class Real> class Geom_;
...
public:
    ...
    /// Construct a quaternion that rotates a ray about an axis to point as close as possible towards a target point.
    Quat_(const Ray& ray, const Vec3& axis, const Vec3& target);
    ...
    /// To string
    friend StringStream& operator<<(StringStream& os, const Quat_& val)
    {
        os << "(";
        for (int i = 0; i < dim; ++i)
        {
