...
/// Find the minimum of a function using bisection and parabolic interpolation
template<class Real>
class Minimize
{
...
public:
    ...
    /**
      ...
      *
      * If (f0,f1,f2) is monotonic, then [a,b] is subdivided and processed.
      ...
      *     and the max depth is 'bracketMax'.
      * -   if f1 = max(f0,f1,f2), then (f0,f1,f2) brackets a maximum.
      *     The minimum search continues recursively as before on [a,(a+b)/2] and [(a+b)/2,b].
      *
      * \param func
