...
template<class Real>
Real Interp_<Real>::linearAngle(Real t, Real angleStart, Real angleEnd, option<int&> rotSign_)
{    
    int rotSign = 0;
...
template<class Real>
void Interp_<Real>::alignDir(Vec3& dir, const Vec3& targetDir, Real angleAmount, option<int&> rotSign_)
{
    int rotSign = 0;
    if (dir == targetDir)
    {
