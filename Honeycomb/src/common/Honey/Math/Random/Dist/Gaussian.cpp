...
Real Gaussian_<Real>::next() const
{
    ...
    RandomGen::GaussianState& state = getGen().getState();

    // Check if deviate is stored
    if (state.deviate == numeral(state.deviate).inf())
    {    
        ...
        state.deviate = dist*Trig_d::cos(angle);
        
        // calculate and return second deviate
        Double deviate = dist*Trig_d::sin(angle);
        return deviate*sigma + mu;
    }
    // If a deviate is available from a previous call to this function, it is returned
    else
    {
        Double deviate = state.deviate;
        state.deviate = numeral(state.deviate).inf();
        return deviate*sigma + mu;
    }
}
