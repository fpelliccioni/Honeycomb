...
private:
    struct Node : MemNode
    {
        ...
        struct Link : MemLink<Node>
        {
            ...
            Link()                                          { data = 0; }
            Link(Node* ptr, bool d = false)                 { data = reinterpret_cast<int>(ptr) | (int)d; }
...
public:
    ...
    bool erase(Iter& it, option<Data&> data = optnull)
    {
        bool erased = false;
        Node* node = it._cur;
...
private:
    ...
    Node& createNode(const Data& data)
    {
        ...
        assert((reinterpret_cast<int>(&node) & ~Link::ptr_mask) == 0, "Pointer not 2-byte aligned, bits found outside mask.");
        node.prev = Link();
        node.next = Link();
        node.data = data;
        return node;
