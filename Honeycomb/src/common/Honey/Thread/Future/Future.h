...
namespace future { namespace priv
{
    ...
    template<class R> struct unwrapOnReady<Future<R>> : mt::FuncptrBase, SmallAllocatorObject
    {
        ...
        {
            if (src.ready) src.e ? promise.setException(*src.e) : promise.setValue(move(static_cast<State<R>&>(src).result()));
            delete_(this);
        }
        Promise<R> promise;
    };
    ...
    template<> struct unwrapOnReady<Future<void>> : mt::FuncptrBase, SmallAllocatorObject
    {
        ...
        void operator()(StateBase& src)
        {
            if (src.ready) src.e ? promise.setException(*src.e) : promise.setValue();
            delete_(this);
        }
...
public:
    ...
    Future(Future&& rhs)                                        : _state(nullptr) { operator=(move(rhs)); }
